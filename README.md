...
这个阶段不追求复杂能力，重点是把“用户输入 -> 模型调用 -> 输出回复”这条链路真正做成可交互产品形态。

### Todo

1. 把 `src/index.js` 从固定消息 demo 改成可交互 CLI
...
6. 增加 `exit / quit` 退出指令
7. 增加启动提示和交互提示
8. 完善错误提示，覆盖配置、鉴权、配额、网络和代理问题
9. 为 CLI 层抽出单独模块，避免交互逻辑继续堆在入口文件
10. 完成后更新 README 当前进度

### 建议顺序

1. 先把 `src/index.js` 改成可交互 CLI
2. 再抽 prompt 和 chat 逻辑
3. 再补连续对话和退出指令
4. 最后整理错误提示和 README

### 验收标准

当下面这些条件都满足时，可以认为 `Phase 1` 完成：
